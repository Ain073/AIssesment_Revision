@foreach ($programs as $program)
    ...
    <div class="modal fade" id="editProgramModal{{ $program->program_id }}" tabindex="-1" aria-labelledby="editProgramModalLabel{{ $program->program_id }}" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <form action="{{ route('department-chair.programs.update', $program) }}" class="modal-content" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="modal-body">
                    <div class="mb-3">
                        ...
                        <div class="form-control form-control-lg bg-light">
                            {{ $scopedDepartment?->dept_name ?? 'No department assigned' }}
                        </div>
                    </div>
                    <div class="mb-3">
    ...
    <div class="modal fade" id="deleteProgramModal{{ $program->program_id }}" tabindex="-1" aria-labelledby="deleteProgramModalLabel{{ $program->program_id }}" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <form action="{{ route('department-chair.programs.destroy', $program) }}" class="modal-content" method="POST">
                @csrf
                @method('DELETE')
                <div class="modal-header">
